...
ZIP="dist/CodeDen-v0.1.7-mac.zip"   # 纯英文名（GitHub 资产名会丢掉中文字符）
...
# 发布 release + 上传安装包
gh release view v0.1.7 >/dev/null 2>&1 || \
  gh release create v0.1.7 "$ZIP" \
    --title "码岛 CodeDen v0.1.7" \
    --notes "v0.1.7：修复 Claude Desktop 历史会话已停止后，最近会话仍显示 思考中... 的问题；Stop 没有 assistant 文本时会正确收尾为已完成，随后转为空闲态。
